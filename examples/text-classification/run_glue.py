...
class ModelArguments:
    ...
    apply_expert_soup: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to apply expert soup or not."},
    )
    expert_soup_path: Optional[str] = field(
        default=None,
        metadata={"help": "The file path of adapter parameters."},
    )
    ...
    use_consistency_loss: Optional[int] = field(
        default=0,
        metadata={"help": "Whether to apply bitfit or not."},
    )
    sharing_down: Optional[int] = field(
        default=0,
        metadata={"help": "Whether to sharing down projection for expert or not."},
    )
    sharing_up: Optional[int] = field(
        default=0,
        metadata={"help": "Whether to sharing down projection for expert or not."},
    )
    weight_strategy: Optional[str] = field(
        default="soft",
        metadata={"help": "Whether to sharing down projection for expert or not."},
    )
...
def main():
    ...
    config = AutoConfig.from_pretrained(
        ...
        apply_expert_soup=model_args.apply_expert_soup,
        num_experts=model_args.num_experts,
        inference_level=model_args.inference_level,
        sharing_down=model_args.sharing_down,
        sharing_up=model_args.sharing_up,
        weight_strategy=model_args.weight_strategy,
        sparsity=model_args.sparsity,
        use_consistency_loss=model_args.use_consistency_loss,
    ...
    # if training_args.do_expert_weight_train and model_args.inference_level == 4:
    #     import pdb
    #     pdb.set_trace()
    #     model = trainer.model
    #
    #     trainable_params.append('expert_score_weight')
    #     fixed_params = []
    #
    #     if len(trainable_params) > 0:
    #         for name, param in model.named_parameters():
    ...
    #                 param.requires_grad = True
    #
    #     training_args.num_train_epochs = training_args.num_train_expert_epochs
    #     training_args.learning_rate = training_args.num_train_expert_epochs.expert_learning_rate
    #
    #     # Initialize our Trainer
    #     trainer = Trainer(
    #         model=model,
    ...
    #     trainer.save_model()  # Saves the tokenizer too for easy upload
    #
    #     trainer.log_metrics("expert_train", metrics)
