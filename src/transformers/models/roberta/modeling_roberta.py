...
class RobertaSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if config.apply_lora:
            self.query = lora.Linear(config.hidden_size, self.all_head_size, config.lora_r,
        ...
        if config.apply_lora:
            self.value = lora.Linear(config.hidden_size, self.all_head_size, config.lora_r,
                                     lora_alpha=config.lora_alpha)
        else:
...
class RobertaForSequenceClassification(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.use_consistency_loss == 1 and self.roberta.training:
            ...
            sequence_output = outputs[0]
            logits_additional = self.classifier(sequence_output)
            if labels is not None:
                loss += symmetric_KL_loss(logits, logits_additional)
            #     if self.num_labels == 1:
            #         #  We are doing regression
            #         loss_fct = MSELoss()
            #         loss += loss_fct(logits_additional.view(-1), labels.view(-1))
            #     else:
            #         loss_fct = CrossEntropyLoss()
